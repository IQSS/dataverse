...
_usage() {
  ...
  echo "  -j     Path (and filename) of java keystore to be created. [${KEYSTORE_PATH}]"
...
while getopts :c:e:E:j:p:v:x:h FLAG; do
  case $FLAG in
    c) 
...
$_IF_VERBOSE eval "$_keytool -keystore dataverse_truststore.jks -importcert -alias $HOSTNAME -file $TLS_CERTIFICATE_PATH -storepass $KEYSTORE_PASSWORD"
