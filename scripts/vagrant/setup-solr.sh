...
#su $GLASSFISH_USER -s /bin/sh -c "echo $PWD"
su $GLASSFISH_USER -s /bin/sh -c "cd $SOLR_HOME/solr-7.2.1/server/solr && cp -r configsets/_default . && mv _default collection1"
...
su $GLASSFISH_USER -s /bin/sh -c "cd $SOLR_HOME/solr-7.2.1 && bin/solr start && bin/solr create_core -c collection1 -d server/solr/collection1/conf/"
