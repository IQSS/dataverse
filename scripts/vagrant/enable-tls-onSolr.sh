...
if [[ ( -z ${KEYSTORE_PATH} ) || ( ! -e $KEYSTORE_PATH ) ]]; then 
  ...
  if [[ -n ${KEYSTORE_PASSWORD} ]]; then
    SOLR_SSL_SETTINGS="${SOLR_SSL_SETTINGS}SOLR_SSL_KEY_STORE_PASSWORD=${KEYSTORE_PASSWORD}\n"
  fi
  if [[ -n ${TRUSTSTORE_PATH} ]]; then
    SOLR_SSL_SETTINGS="${SOLR_SSL_SETTINGS}SOLR_TRUST_STORE=${TRUSTSTORE_PATH}\n"
  fi
  if [[ -n ${TRUSTSTORE_PASSWORD} ]]; then
    SOLR_SSL_SETTINGS="${SOLR_SSL_SETTINGS}SOLR_TRUST_STORE_PASSWORD=${TRUSTSTORE_PASSWORD}\n"
  fi
  if [[ -n ${SSL_NEED_CLIENT_AUTH} ]]; then
    SOLR_SSL_SETTINGS="${SOLR_SSL_SETTINGS}SOLR_SSL_NEED_CLIENT_AUTH=true\n"
  elif [[ -n ${SSL_WANT_CLIENT_AUTH} ]]; then
    SOLR_SSL_SETTINGS="${SOLR_SSL_SETTINGS}SOLR_SSL_WANT_CLIENT_AUTH=true\n"
  fi
fi
