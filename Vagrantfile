...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.define "standalone", primary: true do |standalone|
    ...
    config.vm.provision "shell", path: "scripts/vagrant/setup.sh", args: "-v 3"
    config.vm.provision "shell", path: "scripts/vagrant/setup-solr.sh"
    config.vm.provision "shell", path: "scripts/vagrant/install-dataverse.sh", args: "-m #{mailserver} -v 3"
