    ...
    $(skosmosInputSelector).each(function() {
      ...
      if(!skosmosInput.hasAttribute('data-skosmos')) {
        ...
        // ToDo - get vocab selection
            ...
            $(skosmosInput).before($('<div/>').addClass('cvoc-vocab col-sm-3'));
            $(skosmosInput).parent().find('.cvoc-vocab').append($('<label/>').text('Vocabulary'));
            $(skosmosInput).parent().find('.cvoc-vocab').append(
        ...
        if(Object.keys(vocabs).length == 1 || $(skosmosInput).val().startsWith("http")) {
            $(skosmosInput).parent().find('.cvoc-vocab').hide();
        }
        // Add a select2 element to allow search and provide a list of choices
        var selectId = "skosmosAddSelect_" + num;
        if($(skosmosInput).parent().find('.cvoc-vocab').length!=0) {
          $(skosmosInput).parent().find('.cvoc-vocab').after($('<div/>').addClass('cvoc-term col-sm-9').append($('<label/>').text('Term')).append(
          '<select id=' + selectId + ' class="form-control add-resource select2" tabindex="-1" aria-hidden="true" style="width: 400px">'));
          $('#' + selectId).attr('data-cvoc-cur-vocab', vocab);
          if(Object.keys(vocabs).length == 1 || $(skosmosInput).val().startsWith("http")) {
            $(skosmosInput).parent().find('.cvoc-term > label').hide();
        ...
        $("#" + selectId).select2({
            ...
            allowClear : true,
            ajax : {
              url: function() { return cvocUrl + 'rest/v1/search?unique=true&vocab=' + $('#' + selectId).attr('data-cvoc-cur-vocab') + termParentUri + langParam;},
              dataType: "json",
...
if(Object.keys(vocabs).length > 1) {
      ...
      $('#' + selectId).on('select2:clear', function (e) {
        ...
        $('#' + selectId).text('');
if(Object.keys(vocabs).length > 1) {
            $(skosmosInput).parent().find('.cvoc-vocab').show();
            $(skosmosInput).parent().find('.cvoc-term > label').show();
