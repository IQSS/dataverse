...
/**
 * ingest plugin for Stata 15 (119) DTA file format. A copy and paste from
 * DTA118FileReader with a couple tweaks to how we read varNumber (4 instead of
 * 2) and sortlist.
 *
 ...
 *
 * TODO: There is so much common code with DTA118FileReader and DTA117FileReader
 * that we should figure out how to refactor it.
 */
public class DTA119FileReader extends TabularDataFileReader {
    //@Inject
    ...
    private static final String TAG_DTA_119 = "stata_dta";
    ...
    private static Map<String, Integer> release119constant
            = new LinkedHashMap<String, Integer>();

    private static Map<String, Integer> byteLengthTable119
            = new HashMap<String, Integer>();

    private static Map<Integer, String> variableTypeTable119
    ...
    static {
        ...
        STATA_RELEASE_NUMBER.put(119, "v.14");

        release119constant.put("HEADER", LENGTH_HEADER[1]);
        release119constant.put("LABEL", LENGTH_LABEL[1]);
        release119constant.put("NAME", LENGTH_NAME[1]);
        release119constant.put("FORMAT", LENGTH_FORMAT_FIELD[1]);
        release119constant.put("EXPANSION", LENGTH_EXPANSION_FIELD[2]);
        release119constant.put("DBL_MV_PWR", DBL_MV_PWR[1]);

        CONSTANT_TABLE.put(119, release119constant);

        // 1, 2 and 4-byte integers: 
        byteLengthTable119.put("Byte", 1);
        byteLengthTable119.put("Integer", 2);
        byteLengthTable119.put("Long", 4);
        // 4 and 8-byte floats: 
        byteLengthTable119.put("Float", 4);
        byteLengthTable119.put("Double", 8);
        ...
        byteLengthTable119.put("STRL", 8);

        variableTypeTable119.put(65530, "Byte");
        variableTypeTable119.put(65529, "Integer");
        variableTypeTable119.put(65528, "Long");
        variableTypeTable119.put(65527, "Float");
        variableTypeTable119.put(65526, "Double");

        //variableTypeTable119.put(32768,"STRL");
    }
    ...
    // Constructor -----------------------------------------------------------//
    public DTA119FileReader(TabularDataFileReaderSpi originator) {
        super(originator);
    }
    ...
    private void init() throws IOException {
        ...
        variableTypeTable = variableTypeTable119;

        byteLengthTable = byteLengthTable119;
    ...
    public TabularDataIngest read(BufferedInputStream stream, File dataFile) throws IOException {
        logger.info("DTA119FileReader: read() start");
        ...
        try {
            ...
            dataReader.readOpeningTag(TAG_DTA_119);
            ...
            if (checkClosingTag) {
                dataReader.readClosingTag(TAG_DTA_119);
            }
        ...
        logger.info("DTA119FileReader: read() end.");
        return ingesteddata;
    }
    ...
    private void readHeader(DataReader dataReader) throws IOException {
        ...
        if (!"119".equals(dtaVersionTag)) {
            throw new IOException("Unexpected version tag found: " + dtaVersionTag + "; expected value: 119.");
        }
        ...
        // Note that from 118 to 119 we changed this from 2 to 4. --pdurbin
        int varNumber = dataReader.readIntegerSection(TAG_HEADER_VARNUMBER, 4);
...
//        int obsNumber = dataReader.readIntegerSection(TAG_HEADER_OBSNUMBER, 4);
        ...
        if (datasetTimeStamp == null
            ...
            throw new IOException("unexpected/invalid length of the time stamp in the DTA119 header.");
        } else {
...
//            String variableName = reader.readString(33);
    ...
    private void readSortOrder(DataReader reader) throws IOException {
        ...
        for (int i = 0; i < dataTable.getVarQuantity(); i++) {
            // Note that from 118 to 119 we changed readShortInteger to readInteger(4) --pdurbin
//            int order = reader.readShortInteger();
            int order = reader.readInteger(4);
...
//        int terminatingShort = reader.readShortInteger();
        // Note that from 118 to 119 we changed readShortInteger to readInteger(4) --pdurbin
        int terminatingShort = reader.readInteger(4);
...
//            String variableLabel = reader.readString(81);
    ...
    private void readData(DataReader reader) throws IOException {
        ...
        logger.fine("DTA119 Ingest: readData(): end.");
...
//            if (n != 2 && n != 4) {
        ...
        /* 
         * More complex helper methods for reading DTA119 "sections" ...
         * TODO: document this ...
         */
