...
/**
 * ingest plugin for Stata 14 (118) DTA file format. A copy and paste from
 * DTA117FileReader with some tweaks based on changes in the Stata format:
 *
 * - v.13/("dta 117"): https://www.stata.com/help.cgi?dta_117
 *
 * - v.14/("dta 118"): https://www.stata.com/help.cgi?dta
 */
public class DTA118FileReader extends TabularDataFileReader {
    //@Inject
    ...
    private static final String TAG_DTA_118 = "stata_dta";
    ...
    private static Map<String, Integer> release118constant
            = new LinkedHashMap<String, Integer>();

    private static Map<String, Integer> byteLengthTable118
            = new HashMap<String, Integer>();

    private static Map<Integer, String> variableTypeTable118
    ...
    static {
        ...
        STATA_RELEASE_NUMBER.put(118, "v.14");

        release118constant.put("HEADER", LENGTH_HEADER[1]);
        release118constant.put("LABEL", LENGTH_LABEL[1]);
        release118constant.put("NAME", LENGTH_NAME[1]);
        release118constant.put("FORMAT", LENGTH_FORMAT_FIELD[1]);
        release118constant.put("EXPANSION", LENGTH_EXPANSION_FIELD[2]);
        release118constant.put("DBL_MV_PWR", DBL_MV_PWR[1]);

        CONSTANT_TABLE.put(118, release118constant);

        // 1, 2 and 4-byte integers: 
        byteLengthTable118.put("Byte", 1);
        byteLengthTable118.put("Integer", 2);
        byteLengthTable118.put("Long", 4);
        // 4 and 8-byte floats: 
        byteLengthTable118.put("Float", 4);
        byteLengthTable118.put("Double", 8);
        ...
        byteLengthTable118.put("STRL", 8);

        variableTypeTable118.put(65530, "Byte");
        variableTypeTable118.put(65529, "Integer");
        variableTypeTable118.put(65528, "Long");
        variableTypeTable118.put(65527, "Float");
        variableTypeTable118.put(65526, "Double");

        //variableTypeTable118.put(32768,"STRL");
    }
    ...
    // Constructor -----------------------------------------------------------//
    public DTA118FileReader(TabularDataFileReaderSpi originator) {
        super(originator);
    }
    ...
    private void init() throws IOException {
        ...
        variableTypeTable = variableTypeTable118;

        byteLengthTable = byteLengthTable118;
    ...
    public TabularDataIngest read(BufferedInputStream stream, File dataFile) throws IOException {
        logger.info("DTA118FileReader: read() start");
        ...
        try {
            ...
            dataReader.readOpeningTag(TAG_DTA_118);
            ...
            if (checkClosingTag) {
                dataReader.readClosingTag(TAG_DTA_118);
            }
        ...
        logger.info("DTA118FileReader: read() end.");
        return ingesteddata;
    }
    ...
    private void readHeader(DataReader dataReader) throws IOException {
        ...
        if (!"118".equals(dtaVersionTag)) {
            throw new IOException("Unexpected version tag found: " + dtaVersionTag + "; expected value: 118.");
        }
        ...
        // TODO switch back to fine pdurbin
...
//        int obsNumber = dataReader.readIntegerSection(TAG_HEADER_OBSNUMBER, 4);
         ...
         *
         *
         * Where N = bbbbbbbb is an 8-byte unsigned integer field recorded
         * according to byteorder. In format 117, N was written in a 4-byte
         * field.
         */
        int obsNumber = dataReader.readIntegerSection(TAG_HEADER_OBSNUMBER, 8);
        ...
        // Changing from 80 to 320 --pdurbin
        String datasetLabel = dataReader.readLabelSection(TAG_HEADER_FILELABEL, 320);
        ...
        if (datasetTimeStamp == null
            ...
            throw new IOException("unexpected/invalid length of the time stamp in the DTA118 header.");
        } else {
...
//            String variableName = reader.readString(33);
            // changing from 33 to 129 --pdurbin
            String variableName = reader.readString(129);
 ...
 /* Variable Formats are used exclusively for time and date variables. 
    ...
    private void readDisplayFormats(DataReader reader) throws IOException {
        logger.info("Formats section; at offset " + reader.getByteOffset() + "; dta map offset: " + dtaMap.getOffset_fmts());
        // TODO: 
        // check that we are at the right byte offset!
        ...
        for (int i = 0; i < dataTable.getVarQuantity(); i++) {
            // Changing from 49 to 57. --pdurbin
//            String variableFormat = reader.readString(49);
            String variableFormat = reader.readString(57);
            ...
            logger.info(i + " th variableFormatKey=" + variableFormatKey);
    ...
    private void readValueLabelFormatNames(DataReader reader) throws IOException {
        logger.info("Category valuable section; at offset " + reader.getByteOffset() + "; dta map offset: " + dtaMap.getOffset_vlblnames());
        // TODO: 
        // check that we are at the right byte offset!
...
//            String valueLabelFormat = reader.readString(33);
            // Changing from 33 to 129 --pdurbin
            String valueLabelFormat = reader.readString(129);
...
//            String variableLabel = reader.readString(81);
            // Changed from 81 to 321 --pdurbin
            String variableLabel = reader.readString(321);
    ...
    private void readData(DataReader reader) throws IOException {
        ...
        logger.fine("DTA118 Ingest: readData(): end.");
    ...
    private void readValueLabels(DataReader reader) throws IOException {
        logger.info("Value Labels section; at offset " + reader.getByteOffset() + "; dta map offset: " + dtaMap.getOffset_vallabs());
        logger.info("readValueLabels(): start.");
...
//            String label_table_name = reader.readString(33);
            // Changing from 33 to 129 --pdurbin
            String label_table_name = reader.readString(129);
            // TODO: 
...
//        private byte readSignedByte2() throws IOException {
//            /* Why not just use readBytes(1) here, you ask?
//             * - Because readBytes() will want to allocate a 
//             * return byte[] buffer of size 1. */
//            byte ret;
...
//        // Note that readByte() returns the value of Java type "short". 
//        // This is to accommodate value larger than 127.
//        private short readByte2() throws IOException {
//            short ret = readSignedByte2();
//
//            if (ret < 0) {
//                ret += 256;
//            }
...
//            if (n != 2 && n != 4) {
        ...
        /* 
         * More complex helper methods for reading DTA118 "sections" ...
         * TODO: document this ...
         */
