...
public class SettingsServiceBean {
    ...
    public enum Key {
        ...
        /**
         * Secret used to sign API URLs. Generated by the server on first use; admins may store
         * their own via the settings API ({@link #validateSettingValue} enforces a minimum
         * length on every write path). Deleting it rotates the secret.
         */
        ApiSigningSecret,
        /**
...
//        PVValidatorExpirationMaxLength,
     ...
     */
    public static void validateSettingValue(String name, String content) {
        if (Key.ApiSigningSecret.toString().equals(name)
                && (content == null || content.length() < ApiSigningSecretServiceBean.MIN_SECRET_LENGTH)) {
            throw new SettingsValidationException("The API signing secret must be at least "
