...
/**
 * Service bean accessing a persistent hash map, used as settings in the
 * application.
 * 
 * @author michael
 */
...
public class SettingsServiceBean {
    ...
    /**
     * Some convenient keys for the settings. Note that the setting's name is really
     * a {@code String}, but it's good to have the compiler look over your shoulder
     * when typing strings in various places of a large app. So there.
     */
    public enum Key {
        AllowApiTokenLookupViaApi,
        /**
         * Ordered, comma-separated list of custom fields to show above the fold on
         * dataset page such as "data_type,sample,pdb"
         */
        ...
        PublicInstall,
        /**
         * Sets the name of your cloud computing environment. For example,
         * "Massachusetts Open Cloud"
         */
        CloudEnvironmentName,
        /**
         * Defines the base for a computing environment URL. The container name will be
         * appended to this on the "Compute" button
         */
        ComputeBaseUrl,
        /**
         * Enables the provenance collection popup. Allows users to store their
         * provenance json and description
         */
        ...
        /**
         * If the data replicated around the world using RSAL (Repository Storage
         * Abstraction Layer) is locally available, this is its file path, such as
         * "/programs/datagrid".
         *
         * TODO: Think about if it makes sense to make this a column in the StorageSite
         * database table.
         */
        LocalDataAccessPath, IdentifierGenerationStyle, OAuth2CallbackUrl, DefaultAuthProvider, FooterCopyright,
        FileFixityChecksumAlgorithm, MinutesUntilConfirmEmailTokenExpires,
        /**
        ...
        /**
         * Revert to MyData *not* using the Solr "permission documents" which was the
         * behavior in Dataverse 4.2. Starting to use Solr permission documents in
         * MyData has been introduced in 4.2.1 as a fix for
         * https://github.com/IQSS/dataverse/issues/2649 where the "File Downloader"
         * role was exposing cards for unpublished datasets when it shouldn't.
         */
        MyDataDoesNotUseSolrPermissionDocs,
        /**
         * In Dataverse 4.7 and earlier, an API token was required to use the Search
         * API. Tokens are no longer required but you can revert to the old behavior by
        ...
        /**
         * A key that, with the right {@link ApiBlockingFilter.BlockPolicy}, allows
         * calling blocked APIs.
         */
        BlockedApiKey,
        ...
        /**
         * For development only (see dev guide for details). Backed by an enum of
        ...
        ApiTermsOfUse,
        /**
         * URL for the application-wide Privacy Policy per installation, linked to from
         * the footer.
         */
        ApplicationPrivacyPolicyUrl,
        /**
         * A boolean defining if indexing and search should respect the concept of
         * "permission root".
         *
         * <p>
         *
         * If we ignore permissionRoot at index time, we should blindly give search
         * ("discoverability") access to people and group who have access defined in a
         * parent dataverse, all the way back to the root.
         *
         * <p>
         *
         * If we respect permissionRoot, this means that the dataverse being indexed is
         * an island of permissions all by itself. We should not look to its parent to
         * see if more people and groups might be able to search the DvObjects within
         * it. We would assume no implicit inheritance of permissions. In this mode, all
         * permissions must be explicitly defined on DvObjects. No implied inheritance.
         *
         */
        ...
        /**
         * Default Key for limiting the number of bytes uploaded via the Data Deposit
         * API, UI (web site and .
         */
        ...
        Shoulder,
        /*
         * Removed for now - tried to add here but DOI Service Bean didn't like it at
         * start-up DoiUsername, DoiPassword, DoiBaseurlstring,
         */
        /** Optionally override http://guides.dataverse.org . */
        GuidesBaseUrl,

        /**
         * A link to an installation of https://github.com/IQSS/miniverse or some other
         * metrics app.
         */
        MetricsUrl,

        /**
         * Number of minutes before a metrics query can be rerun. Otherwise a cached
         * value is returned. Previous month dates always return cache. Only applies to
         * new internal caching system (not miniverse).
         */
        MetricsCacheTimeoutMinutes,
        /* zip download size limit */
        /** Optionally override version number in guides. */
        GuidesVersion, ZipDownloadLimit,
        /* zip upload number of files limit */
        ZipUploadFilesLimit,
        /*
         * the number of files the GUI user is allowed to upload in one batch, via
         * drag-and-drop, or through the file select dialog
         */
        MultipleUploadFilesLimit,
        /* Size limits for generating thumbnails on the fly */
        /*
         * (i.e., we'll attempt to generate a thumbnail on the fly if the size of the
         * file is less than this)
         */
        ...
        /*
         * (can be set separately for specific ingestable formats; in which case the
         * actual stored option will be TabularIngestSizeLimit:{FORMAT_NAME} where
         * {FORMAT_NAME} is the format identification tag returned by the
         * getFormatName() method in the format-specific plugin; "sav" for the SPSS/sav
         * format, "RData" for R, etc. for example: :TabularIngestSizeLimit:RData
         */
        ...
        // Option to overide the feedback dialog display with a link to an external page
        // via its url
        NavbarSupportUrl,

        /**
         * The theme for the root dataverse can get in the way when you try make use of
         * HeaderCustomizationFile and LogoCustomizationFile so this is a way to disable
         * it.
         */
        DisableRootDataverseTheme,
        // Limit on how many guestbook entries to display on the guestbook-responses
        // page:
...
//        PVValidatorExpirationMaxLength,
        ...
        /**
         * Configurable text for alert/info message on passwordreset.xhtml when users
         * are required to update their password.
         */
        PVCustomPasswordResetAlertMessage,
        /*
         * String to describe DOI format for data files. Default is DEPENDENT.
         * 'DEPENEDENT' means the DOI will be the Dataset DOI plus a file DOI with a
        ...
        Languages,
        /*
         * Number for the minimum number of files to send PID registration to
        ...
        /**
         * Indicates if the Handle service is setup to work 'independently' (No
         * communication with the Global Handle Registry)
         */
        IndependentHandleService,

        /**
         * Archiving can be configured by providing an Archiver class name (class must
         * extend AstractSubmitToArchiverCommand) and a list of settings that should be
         * passed to the Archiver. Note: Configuration may also require adding
         * Archiver-specific jvm-options (i.e. for username and password) in glassfish.
         * 
         * To automate the submission of an archival copy step as part of publication, a
         * post-publication workflow must also be configured.
         * 
         * For example: ArchiverClassName -
         * "edu.harvard.iq.dataverse.engine.command.impl.DPNSubmitToArchiveCommand"
         * ArchiverSettings - "DuraCloudHost, DuraCloudPort, DuraCloudContext"
         * 
         * Note: Dataverse must be configured with values for these dynamically defined
         * settings as well, e.g.
         * 
         * DuraCloudHost , eg. "qdr.duracloud.org", a non-null value enables submission
         * DuraCloudPort, default is 443 DuraCloudContext, default is "durastore"
         */
        ...
        /**
         * Lifespan, in minutes, of a login user session  (both DataverseSession and the
         * underlying HttpSession)
         */
        LoginSessionTimeout,

        /**
         * Shibboleth affiliation attribute which holds information about the
         * affiliation of the user (e.g. ou)
         */
        ...
        /**
         * If defined, this is the URL of the zipping service outside the main
         * Application Service where zip downloads should be directed instead of
         * /api/access/datafiles/
         */
        CustomZipDownloadServiceUrl,
        /**
         * Sort Date Facets Chronologically instead or presenting them in order of # of
         * hits as other facets are. Default is true
         */
        ChronologicalDateFacets,

        /**
         * Used where BrandingUtil.getInstallationBrandName is called, overides the
         * default use of the root Dataverse collection name
         */
        InstallationName,
        /**
    ...
    /**
     * Attempt to convert the value to an integer - Applicable for keys such as
     * MaxFileUploadSizeInBytes
     * 
    ...
    /**
     * Attempt to convert a value in a compound key to a long - Applicable for keys
     * such as MaxFileUploadSizeInBytes after multistore capabilities were added in
     * ~v4.20 backward compatible with a single value. For multi values, the key's
     * value must be an object with param:value pairs. A "default":value pair is
     * allowed and will be returned for any param that doesn't have a defined value.
     * 
     * On failure (key not found or string not convertible to a long), returns null
     * 
     * @param key
     ...
     * name exists. The main difference between this method and the other
     * {@code get()}s is that is never returns null (unless {@code defaultValue} is
     * {@code null}.
     * 
    ...
    public String get(String name, String lang, String defaultValue) {
        List<Setting> tokens = em.createNamedQuery("Setting.findByNameAndLang", Setting.class)
                .setParameter("name", name).setParameter("lang", lang).getResultList();
        String val = null;
        if (tokens.size() > 0) {
            val = tokens.get(0).getContent();
        }
    ...
    /**
     * The correct way to decide whether a string value in the settings table should
     * be considered as {@code true}.
     * 
     * @param name         name of the setting.
    ...
    public void delete(String name, String lang) {
        actionLogSvc.log(new ActionLogRecord(ActionLogRecord.ActionType.Setting, "delete").setInfo(name));
        em.createNamedQuery("Setting.deleteByNameAndLang").setParameter("name", name).setParameter("lang", lang)
                .executeUpdate();
    }
