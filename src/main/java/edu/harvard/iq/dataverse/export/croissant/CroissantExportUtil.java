...
public class CroissantExportUtil {
    ...
    public static void exportDataset(
        ...
        try {
            ...
            if (datasetType != null && DatasetType.DATASET_TYPE_REVIEW.equals(datasetType)) {
                /**
                 * Review datasets are "inside out" in the sense that we don't present the normal
                 * metadata fields via Croissant. Instead, we attempt to represent the item being
                 * reviewed. The number of fields we can expose is limited, but we can expose
                 * itemReviewedUrl, for example.
                 */
                job.add(
                        "url",
                        describes
                                .getJsonObject("review:itemReviewed")
                                .getJsonString("review:itemReviewedUrl"));
            } else {
                // This is something other than a review dataset. Display all the normal Croissant
                // fields.
                 ...
                 * Croissant validator throws a warning for anything other than MAJOR.MINOR.PATCH.
                 * See the README for a detailed explanation and the following issues:
                 ...
                 * dataset itself into "citeAs". However, the spec says "citeAs" is "A citation for
                 * a publication that describes the dataset" so we have asked for clarification
                 * here: https://github.com/mlcommons/croissant/issues/638
                 */
                ...
                for (int i = 0; i < oreFiles.size(); i++) {
                    ...
                    String storageIdentifier =
                            aggregate.getString("dvcore:storageIdentifier", null);
                ...
                for (JsonValue jsonValue : datasetFileDetails) {
                     ...
                     * ingest and that multiple files formats are available: original,
                     * tab-separated, and RData. Currently we are only showing the original file but
                     * we we could create additional cr:FileObject entries for tab-separated and
                     * RData as suggested in https://github.com/mlcommons/croissant/issues/641 .
                     * Should we? Is there interest in this? And would we duplicate all the
                     * cr:RecordSet entries (columns) with each additional format? Probably not as
                     ...
                     * don't include a unit because the spec says "Defaults to bytes if a unit is
                     * not specified."
                     */
                    ...
                    for (JsonValue dataTableValue : dataTables) {
                        ...
                        recordSetContent.add(
                                ...
                                Json.createObjectBuilder()
                                        ...
                                        .add(
                                                "dataType",
                                                "http://www.wikidata.org/entity/Q4049983"));
                        ...
                        for (JsonValue dataVariableValue : dataVariables) {
                            ...
                            String variableName =
                                    StringEscapeUtils.escapeHtml4(
                                            dataVariableObject.getString("name"));
                            ...
                            if (variableSummaryStatistics != null) {
                                // Same order as upstream: MEAN, MEDN, MODE, MIN, MAX, STDEV, VALD,
                                // INVD
                                annotationsBuilder
                                        .add(
                                                Json.createObjectBuilder()
