...
public class OAI_OREExporter implements Exporter {
    ...
    public void exportDataset(DatasetVersion version, JsonObject json, OutputStream outputStream)
        ...
        try {
            new OREMap(version, ExportService.settingsService.isTrueForKey(SettingsServiceBean.Key.ExcludeEmailFromExport, false), OREMap.ARCHIVE).writeOREMap(outputStream);
        } catch (Exception e) {
