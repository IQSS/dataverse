...
public class MailServiceBean implements java.io.Serializable {
	...
	public void sendMail(String host, String reply, String to, String subject, String messageText) {
		...
		try {
			...
			try {
				...
				fromAddress.setPersonal(BundleUtil.getStringFromBundle("contact.delegation",
						Arrays.asList(fromAddress.getPersonal(), reply)), charset);
	...
	public void sendMail(String reply, String to, String subject, String messageText,
		...
		try {
			...
			if (EMailValidator.isEmailValid(reply, null)) {
				// But set the reply-to address to direct replies to the requested 'from' party
		...
		if (emailAddress != null) {
			...
			if (objectOfNotification != null) {
				...
				if (!(messageText.isEmpty() || subjectText.isEmpty())) {
					...
					logger.warning(
							"Skipping " + notification.getType() + " notification, because couldn't get valid message");
				}
			} else {
				logger.warning(
						"Skipping " + notification.getType() + " notification, because no valid Object was found");
			}
		} else {
	 ...
	 * 
	 * @param au
	 *            The authenticated user whose role assignments we look for.
	 * @param dvObj
	 *            The Dataverse object over which the roles are assigned
	 ...
	 * 
	 * @param d
	 *            The Dataverse object to get a link for.
	 ...
	 * 
	 * @param d
	 *            The Dataverse object to get the string for
	...
	public String getMessageTextBasedOnNotification(UserNotification userNotification, Object targetObject,
		...
		case ASSIGNROLE:
			...
			if (joinedRoleNames.contains("File Downloader")) {
				if (dvObjTypeStr.equals("dataset")) {
					pattern = ResourceBundle.getBundle("Bundle")
							.getString("notification.access.granted.fileDownloader.additionalDataset");
				...
				if (dvObjTypeStr.equals("dataverse")) {
					pattern = ResourceBundle.getBundle("Bundle")
							.getString("notification.access.granted.fileDownloader.additionalDataverse");
		...
		case CREATEDV:
			...
			// initialize to empty string in the rare case that there is no parent dataverse
			// (i.e. root dataverse just created)
			...
			String dataverseCreatedMessage = BundleUtil.getStringFromBundle("notification.email.createDataverse",
					Arrays.asList(dataverse.getDisplayName(), getDataverseLink(dataverse), parentDataverseDisplayName,
							parentDataverseUrl, systemConfig.getGuidesBaseUrl(), systemConfig.getGuidesVersion()));
		...
		case REQUESTFILEACCESS:
			...
			String[] paramArrayRequestFileAccess = { datafile.getOwner().getDisplayName(),
		...
		case CREATEDS:
			...
			String datasetCreatedMessage = BundleUtil.getStringFromBundle("notification.email.createDataset",
					Arrays.asList(version.getDataset().getDisplayName(), getDatasetLink(version.getDataset()),
							version.getDataset().getOwner().getDisplayName(),
							getDataverseLink(version.getDataset().getOwner()), systemConfig.getGuidesBaseUrl(),
		...
		case MAPLAYERUPDATED:
			...
			String[] paramArrayMapLayer = { version.getDataset().getDisplayName(),
		...
		case SUBMITTEDDS:
			...
			/*
			 * FIXME Setting up to add single comment when design completed
			 * "submissionComment" needs to be added to Bundle mightHaveSubmissionComment =
			 * "."; if (comment != null && !comment.isEmpty()) { mightHaveSubmissionComment
			...
			String[] paramArraySubmittedDataset = { version.getDataset().getDisplayName(),
					getDatasetDraftLink(version.getDataset()), version.getDataset().getOwner().getDisplayName(),
					getDataverseLink(version.getDataset().getOwner()), mightHaveSubmissionComment };
		...
		case PUBLISHEDDS:
			...
			String[] paramArrayPublishedDataset = { version.getDataset().getDisplayName(),
					getDatasetLink(version.getDataset()), version.getDataset().getOwner().getDisplayName(),
		...
		case RETURNEDDS:
			...
			/*
			 * FIXME Setting up to add single comment when design completed
			 * optionalReturnReason = "."; if (comment != null && !comment.isEmpty()) {
			 * optionalReturnReason = ".\n\n" +
			 * BundleUtil.getStringFromBundle("wasReturnedReason") + "\n\n" + comment; }
			 */
			String[] paramArrayReturnedDataset = { version.getDataset().getDisplayName(),
					getDatasetDraftLink(version.getDataset()), version.getDataset().getOwner().getDisplayName(),
					getDataverseLink(version.getDataset().getOwner()), optionalReturnReason };
		...
		case CHECKSUMFAIL:
			...
			String checksumFailMsg = BundleUtil.getStringFromBundle("notification.checksumfail",
					Arrays.asList(dataset.getGlobalId()));
		...
		case FILESYSTEMIMPORT:
			...
			String fileImportMsg = BundleUtil.getStringFromBundle("notification.mail.import.filesystem",
					Arrays.asList(systemConfig.getDataverseSiteUrl(), version.getDataset().getGlobalId(),
		...
		case CHECKSUMIMPORT:
			...
			String checksumImportMsg = BundleUtil.getStringFromBundle("notification.import.checksum",
					Arrays.asList(version.getDataset().getGlobalId(), version.getDataset().getDisplayName()));
