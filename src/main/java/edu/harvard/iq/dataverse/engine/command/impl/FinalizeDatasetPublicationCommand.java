...
public class FinalizeDatasetPublicationCommand extends AbstractPublishDatasetCommand<Dataset> {
    ...
    public Dataset execute(CommandContext ctxt) throws CommandException {
        ...
        /*
         * Try to register the dataset identifier, and file pids if enabled and needed.
         * For PID providers that have registerWhenPublished == false (all except the
         * FAKE provider at present) the registerExternalIdentifier command will make
         * one try to create the identifier if needed (e.g. if reserving at dataset
         * creation wasn't done/failed). For registerWhenPublished == true providers, if
         * a PID conflict is found, the call will retry with new PIDs.
         */
        ...
        if (theDataset.getPublicationDate() == null) {
            ...
            for (DataFile dataFile : theDataset.getFiles()) {
                ...
                if (embargo != null) {
                    // "dataAvailable" is not nullable in the Embargo class, no need for a null
                    // check
        ...
        // Use dataset pub date (which may not be the current date for migrated
        // datasets)
        ...
        // Remove any pre-pub workflow lock (not needed as
        // WorkflowServiceBean.workflowComplete() should already have removed it after
        // setting the finalizePublication lock?)
    ...
    // These notification methods are fairly similar, but it was cleaner to create a
    // few copies. If more notifications are needed in this command, they should
