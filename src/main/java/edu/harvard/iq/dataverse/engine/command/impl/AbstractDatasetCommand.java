...
//                }
                try {
                    if (!globalIdServiceBean.ableToRegister(theDataset.getGlobalId())) {
                        int attempts = 0;
                        if(retry) {
                            do  {
                                ...
                                attempts++;
                            } while ((!globalIdServiceBean.ableToRegister(theDataset.getGlobalId())) && attempts <= FOOLPROOF_RETRIAL_ATTEMPTS_LIMIT);
                        }
                        if(!retry) {
