...
/**
 * Used to identify, "repackage", and extract data from Shapefiles in .zip
 * format
 *
 * (1) Identify if a .zip contains a shapefile: boolean
 * containsShapefile(FileInputStream zipStream) or boolean
 * containsShapefile(FileInputStream zip_filename)
 *
 *
 *
 * (2) Unpack/"Repackage" .zip: (a) All files extracted (b) Each group of files
 * that make up a shapefile are made into individual .zip files (c) Non
 * shapefile-related files left on their own
 *
 * If the original .zip contains: "shape1.shp", "shape1.shx", "shape1.dbf",
 * "shape1.prj", "shape1.ain", "shape1.aih", "shape2.shp", "shape2.shx",
 * "shape2.dbf", "shape2.prj", "shape1.pdf", "README.md", "shape_notes.txt" The
 * repackaging results in a folder containing: "shape1.zip", "shape2.zip",
 * "shape1.pdf", "README.md", "shape_notes.txt"
 *
 * Code Example: FileInputStream shp_file_input_stream = new FileInputStream(new
 * File("zipped_shapefile.zip")) ShapefileHandler shp_handler = new
 * ShapefileHandler(shp_file_input_stream); if
 * (shp_handler.containsShapefile()){ File rezip_folder = new
 * File("~/folder_for_rezipping"); boolean rezip_success =
 * shp_handler.rezipShapefileSets(shp_file_input_stream, rezip_folder ); if
 * (!rezip_success){ // rezip failed, should be an error message (String)
 * available System.out.println(shp_handler.error_message); } }else{ if
 * (shp_handler.errorFound){ System.out.println("Error message: " +
 * shp_handler.error_message; } }
 *
 *
 * @author raprasad
 *
...
public class ShapefileHandler {
    ...
    /**
     * Update the fileGroup hash which contains a { base_filename : [ext1, ext2,
     * etc ]} This is used to determine whether a .zip contains a shapefile set
     * #
     *
    ...
    /**
     * ************************************
     * Iterate through the zip file contents. Does it contain any shapefiles?
     *
    ...
    public static void main(String[] args) {
        ...
        if (args.length == 0) {
            ...
            logger.severe("Please only give one file name!");
        } else {
            /*
