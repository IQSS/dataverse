...
public class Datasets extends AbstractApiBean {
    ...
    public Response getDataset(@PathParam("id") String id, @Context UriInfo uriInfo, @Context HttpHeaders headers, @Context HttpServletResponse response) {
        return response(req -> {
            ...
            // Report MDC if this is a released version (could be draft if user has access,
            // or user may not have access at all and is not getting metadata beyond the
            // minimum)
    ...
    public Response exportDataset(@QueryParam("persistentId") String persistentId, @QueryParam("exporter") String exporter, @Context UriInfo uriInfo, @Context HttpHeaders headers, @Context HttpServletResponse response) {
        ...
        try {
            ...
            // Export is only possible for released (non-draft) dataset versions so we can
            // log without checking to see if this is a request for a draft
    ...
    public Response deleteDataset(@PathParam("id") String id) {
        // Internally, "DeleteDatasetCommand" simply redirects to
        // "DeleteDatasetVersionCommand"
        ...
        // currently provided by this API is covered between the "deleteDraftVersion"
        // and
    ...
    public Response listVersions(@PathParam("id") String id) {
        return response(req -> ok(execCommand(new ListVersionsCommand(req, findDatasetOrDie(id)))
                .stream()
                .map(d -> json(d))
                .collect(toJsonArray())));
    }
    ...
    public Response getMakeDataCountCitations(@PathParam("id") String idSupplied) {
        ...
        try {
            ...
            for (DatasetExternalCitations citation : externalCitations) {
                ...
                /**
                 * In the future we can imagine storing and presenting more information about
                 * the citation such as the title of the paper and the names of the authors. For
                 * now, we'll at least give the URL of the citation so people can click and find
                 * out more about the citation.
                 */
    ...
    public Response getMakeDataCountMetric(@PathParam("id") String idSupplied, @PathParam("metric") String metricSupplied, @PathParam("yyyymm") String yyyymm, @QueryParam("country") String country) {
        try {
            ...
            if (yyyymm != null) {
                // We add "-01" because we store "2018-05-01" rather than "2018-05" in the
                // "monthyear" column.
                // Dates come to us as "2018-05-01" in the SUSHI JSON ("begin-date") and we
                // decided to store them as-is.
             ...
             * getDatasetMetricsByDatasetMonthCountry returns a single row right now, by
             * country. We could return multiple metrics (viewsTotal, viewsUnique,
             * downloadsTotal, and downloadsUnique) by country.
             */
    ...
    public Response getTimestamps(@PathParam("identifier") String id) {
        ...
        try {
            ...
            if (dataset.isReleased() || canSeeDraft) {
                ...
                // set and the index time is null or is before the mod time, the relevant index
                // is stale
    ...
    /**
     * API to list dataset creation and modification dates, curation status, and
     * direct curator role assignments
     * 
     * Curation Status is a label from the :AllowedCurationLabels setting The
     * returned table/csv file also lists the users with any assignments on this
     * dataset that allow publication. These would be most relevant in uses cases
     * where local role assignments are used to identify responsible party(ies) who
     * have been given roles in movign the dataset through the defined curation
     * process.
     * 
     * @return a CSV table with headings
     * @throws WrappedResponse
     */
    @GET
