...
public record MultiDatasetExportRequest(
    ...
    public record ExportItem(
        @NotBlank
        @Schema(description = "Persistent identifier of the dataset to export.", examples = {"doi:10.5072/FK2/ABCDEF"})
        String persistentId,
        
        @Pattern(
            regexp = ApiConstants.DS_VERSION_IDENTIFIER_REGEX,
            message = "{api.datasets.export.parameters.invalidVersion}"
        )
        @Schema(description = "Dataset version to export. May be a symbolic identifier (" +
                ApiConstants.DS_VERSION_LATEST + ", " + ApiConstants.DS_VERSION_DRAFT + ", " +
                ApiConstants.DS_VERSION_LATEST_PUBLISHED + ") or a numeric version such as 1.0 or 2.3. " +
                "Optional: if omitted, null, or blank, the server substitutes with " + ApiConstants.DS_VERSION_LATEST_PUBLISHED,
            defaultValue = ApiConstants.DS_VERSION_LATEST_PUBLISHED,
            nullable = true)
        String version
    ) {
