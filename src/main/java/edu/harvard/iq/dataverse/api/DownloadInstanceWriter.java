...
public class DownloadInstanceWriter implements MessageBodyWriter<DownloadInstance> {
        ...
        if (di.getDownloadInfo() != null && di.getDownloadInfo().getDataFile() != null) {
            ...
            if (storageIO != null) {
                ...
                if (di.getConversionParam() != null) {
                    ...
                    if (di.getConversionParam().equals("imageThumb") && !dataFile.isHarvested()) { 
                        ...
                        if (di.getConversionParam().equals("noVarHeader")) {
                            ...
                            } else if ("prep_dp".equals(di.getConversionParamValue())) {
                                // Differentially private Data Summary ("preprocessed") metadata
                                // is a special case in that it cannot generated on the file - 
                                // it can only be generated by an external tool, and cached by the 
                                // Dataverse. So we simply check if this cached file exists, 
                                // and return 404 if not:
                                if (storageIO.isAuxObjectCached(di.getConversionParamValue())) {
                                    InputStream is = storageIO.getAuxFileAsInputStream(di.getConversionParamValue());
                                    long size = storageIO.getAuxObjectSize(di.getConversionParamValue());
                                    String fileName = storageIO.getFileName();
                                    storageIO = new InputStreamIO(is, size);
                                    storageIO.setFileName(FileUtil.replaceExtension(fileName, di.getConversionParamValue()));
                                } else {
                ...
                if (instream != null) {
                    ...
                    if (di.getGbr() != null && !(isThumbnailDownload(di) || isSummaryMetadataDownload(di))) {
                        try {
    ...
    private boolean isSummaryMetadataDownload(DownloadInstance downloadInstance) {
        if (downloadInstance == null) return false; 
        ...
        return downloadInstance.getConversionParam().equals("format") && downloadInstance.getConversionParamValue().startsWith("prep");
    }
