...
Conversely, Dataverse can recieve notices from other configured repositories announcing relationships between their publications and datasets. If the referenced dataset exists in the Dataverse instance, a notification will be sent to users who can publish the dataset. They can then decide whether to create a backlink to the publication in the dataset metadata.
...
:LDNTarget and :LDNAnnounceRequiredFields

Configuration to receive messages involves specifying the
:LDNMessageHosts

(FWIW: These settings are not new)
