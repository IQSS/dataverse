...
- **Tree view toggle now appears for flat datasets too.** Previously the JSF Tree/Table toggle (`DatasetPage.isFileTreeViewRequired`) was gated on the dataset having `directoryLabel` set on at least one file. Flat datasets (>1 file, no folders) didn't show the toggle, locking those users into the table view. Since the tree view's bulk-download UX (checkbox selection + client-side streaming zip with per-file resume / two-pass recovery) is a strict upgrade over the legacy server-zipped bulk download regardless of folder structure, the gate now requires only `> 1 file`. Single-file and empty datasets still hide the toggle (no-op).
