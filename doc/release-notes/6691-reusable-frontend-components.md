...
The components are not shipped in the WAR. Build them from `dataverse-frontend`, serve the result as static content, and point `dataverse.reusable-components.base-url` at that path. The flags do nothing until it is set. They also require `dataverse.feature.api-session-auth`, and `dataverse.feature.api-session-auth-hardening` is recommended alongside it. See [Reusable Frontend Components](https://guides.dataverse.org/en/latest/container/running/reusable-components.html) for setup.
