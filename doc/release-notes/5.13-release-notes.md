...
### Select a Folder to Upload (dvwebloader, S3 only)

For installations using S3 for storage and with direct upload enabled, a new tool called [DVWebloader](https://github.com/gdcc/dvwebloader) can be enabled that allows users to upload a folder with a hierarchy of files and subfolders while retaining the relative paths of files. See [Folder Upload](http://preview.guides.gdcc.io/en/develop/user/dataset-management.html#folder-upload) in the User Guide for details. (PR #9096)
...
Licenses as shown in the dropdown in UI can be now sorted by the superusers. See [Sorting Licenses](http://preview.guides.gdcc.io/en/develop/installation/config.html#sorting-licenses) section of the Installation Guide for details. (PR #8697)
...
An [extractNcml](http://preview.guides.gdcc.io/en/develop/api/native-api.html#extract-ncml) API endpoint has been added, especially for installations with existing NetCDF and HDF5 files. After upgrading, they can iterate through these files and try to extract an NcML file.

See the [NetCDF and HDF5](http://preview.guides.gdcc.io/en/develop/user/dataset-management.html#netcdf-and-hdf5) section of the User Guide for details. (PR #9239)
...
External tools can now be configured to use signed URLs to access the Dataverse API as an alternative to API tokens. This eliminates the need for tools to have access to the user's API token in order to access draft or restricted datasets and datafiles. Signed URLs can be transferred via POST or via a callback when triggering a tool via GET. See [Authorization Options](http://preview.guides.gdcc.io/en/develop/api/external-tools.html#authorization-options) in the External Tools documentation for details. (PR #9001)

### Geospatial Search (API Only)

Geospatial search is supported via the Search API using two new [parameters](http://preview.guides.gdcc.io/en/develop/api/search.html#parameters): `geo_point` and `geo_radius`.
...
See [Metadata Blocks](http://preview.guides.gdcc.io/en/develop/api/native-api.html#metadata-blocks-api) in the API Guide for details. (PR #9213)

### Advanced Database Settings

You can now enable advanced database connection pool configurations useful for debugging and monitoring as well as other settings. Of particular interest may be `sslmode=require`. See the new [Database Persistence](http://preview.guides.gdcc.io/en/develop/installation/config.html#database-persistence) section of the Installation Guide for details. (PR #8915)
...
Now that .eln files are recognized, you can run the [Redetect File Type](http://preview.guides.gdcc.io/en/develop/api/native-api.html#redetect-file-type) API on them to switch them from "unknown" to "ELN Archive". Afterward, you can reindex these files to make them appear in search facets.