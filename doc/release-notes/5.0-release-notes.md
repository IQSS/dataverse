...
- Users will be able to upload files with the same name in a dataset, as long as a those files are in different file paths. (Issue #4813, )
- Users will be able to upload files with the same checksum in a dataset.
...
- Users will now have their files validated during publish, and in the unlikely event that anything has happened to the files between deposit and publish, they will be able to take corrective action.
- Administrators will likely see more success with Harvesting, as many minor harvesting issues have been resolved.
- Administrators can now enable an external zip service that frees up application server resources and allows the zip download limit to be increased.
- Administrators can now create groups based on user's email domains. (Issue #6936, PR #6974)
- Administrators can now set date facets to be organized chronologically.
- Administrators can now link harvested datasets using an API (Issue #5886, PR #6935)
- Administrators can now destroy datasets with mapped shapefiles (PR #6860)
