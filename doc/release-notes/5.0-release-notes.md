...
17. (Required for installations using DataCite) Add doi.dataciterestapiurlstring

Add the new JVM option called "doi.dataciterestapiurlstring" with a value of "https://api.datacite.org" for production environments and "https://api.test.datacite.org" for test environments.

"doi.mdcbaseurlstring" should be deleted if it was previously set.

18. (Recommended for installations using DataCite) Pre-register DOIs
...
19. (Experimental) Set up Standalone Zipper Service if desired. Please consult the scripts/zipdownload/README.md in the Dataverse 5 source tree for more information.
