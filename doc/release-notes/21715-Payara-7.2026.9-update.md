...
1. Download the new Payara version 7.2026.9, and unzip it.
   ...
   ```
1. Set ownership of the payara distribution to root, making it read-only for the service account ("dataverse" by default).
...
1. Set ownership of the domain for the service account ("dataverse" by default). Ownership should have been preserved during the move above, but this step assures that the domain is writable by the service account.
