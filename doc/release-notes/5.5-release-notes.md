...
### Improved Workflow around Downloading Large Zip Files

Users trying to download a zip file larger than the Dataverse installation's :zipdownloadlimit will now receive messaging that the zip file is too large and will be presented with alternate access options. Previously, the zip file would download and files above the :zipdownloadlimit would be excluded.
