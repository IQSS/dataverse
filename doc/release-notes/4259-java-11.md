...
- If you're moving from a Dataverse installation that was previously running Glassfish 4.x (typically this would be Dataverse 4.x), you will need to adjust some JVM options in domain.xml as part of the upgrade process. We've provided these optional steps below. These steps are not required if your first installed Dataverse version was running Payara 5.x (typically Dataverse 5.x).
