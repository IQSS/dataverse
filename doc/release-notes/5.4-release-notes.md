...
- Dataverse Administrators can now deactivate users using a new API. (Issue #2419, PR #7629)
...
- API Users will receive Dataverse collection responses in a deterministic order. (Issue #7634, PR #7708)
...
- Dataverse Administrators can now set subfields of compound fields as **conditionally required**, that is, the field is required if and only if any of its "sibling" fields are entered. For example, Producer Name is now conditionally required in the citation metadata block. A user does not have to enter a Producer, but if they do, they have to enter a Producer Name. (Issue #7606, PR #7608)
