...
will output a simple html listing, based on the standard Apache directory index, with Access API download links for individual files, and recursive calls to the API above for sub-folders. (See the documentation entry in the guides for more information).
...
### Java 11, PostgreSQL 13, and Solr 8 Upgrades

Several of the core components of the Dataverse Software have been updated. Specifically:
...
- The Dataverse Software has now been tested with PostgreSQL versions up to 13. Versions 9.6+ should work, but this will extend the version beyond PostgreSQL EOL later in 2021.
...
- Deactivate Users (Issue #2419, PR #7629)
- Remove Roles API (Issue #2419, PR #7629)
- User Traces API (Issue #2419, PR #7629)
- Workflows can now generate messages (Issue #7564, PR #7635)
- Prepublication Workflows speed improvement (Issue #7681, PR #7682)
- Harvest Improvements Support for ISO-639-3 (Issue #7638, PR #7690)
- Order DV Contents in API (Issue #7634, PR #7708)
- Restricted Summary Stats (Issue #7619, PR #7642)
- Truncation (Issue #6685, PR #7312)
- Crawlable File Access (Issue #7084, PR #7579)
- Additional Checksum type support in Direct Upload API (Issue #7600, PR #7602)
- Copy Checksums, Tokens, Private URLs (Issue #6039, Issue #6685, PR #7539, PR #7312)
- Non-ascii characters (Issue #820, PR #7378)
...
Before you start the "index all", Dataverse will appear to be empty because the search results come from Solr. As indexing progresses, partial results will appear until indexing is complete.

See http://guides.dataverse.org/en/5.4/installation/prerequisites.html#installing-solr
...
2\. DB Cleanup for Superusers Releasing without Version Updates
...
3\. (Optional, but recommended) DB Cleanup for Saved Searches and Linked Objects
...
4\. Take a backup of the Worldmap links, if any.
