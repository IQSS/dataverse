...
For installations that were previously running on Dataverse Software 4.x, and have a domain.xml that originated from Glassfish, JVM Options have been added. If your Dataverse installation was first installed on the 5.x series, the JVM options should already be present.

The JVM Options are enumerated in the detailed installation instructions below.
...
3\. **(only required for installations previously running Dataverse Software 4.x!)** In other words, and have a domain.xml that **originated under Glassfish 4**, the below JVM Options need to be added. If your Dataverse installation was first installed on the 5.x series, these JVM options should already be present.
