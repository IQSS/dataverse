...
1. Double check `request-timeout-seconds` setting.
...
1. Deploy the Dataverse 6.12 war file.
   ...
   ```
Please note: when watching logs while waiting for the deployment to finish, you may encounter `SEVERE` level warnings like "Leaking injection manager found in the current thread. Very likely because it wasn't removed in a previous invocation and leaked into this invocation." The Core Team has not found severe issues during testing this release and advice to ignore but monitor for unusual resource usage. We will address the underlying issue with the next release, see #12715 for more details.
