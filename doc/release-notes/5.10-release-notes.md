...
A PostgreSQL upgrade is not required for this release but will be in the future. See below for details.
...
- Administrators can run a version of Solr that doesn't include a version of log4j with known vulnerabilities. (PR #8415)
...
In the "Backward Incompatibilities" section below, note changes in the API regarding licenses.
...
6\. Run ReExportall to update Exports
