...
Fixed an O(N) database write bottleneck during file uploads where `UpdateDatasetVersionCommand` unconditionally touched `modificationTime` on all existing files in the dataset.
...
`UpdateDatasetVersionCommand` now only initializes timestamps on newly created files (`createDate == null`), safely handles null values on existing files, or updates single files when explicit variable metadata (`fmVarMet`) is modified. See #12728.
