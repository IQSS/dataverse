...
(Alternatively, you can try installing Counter Processor in Vagrant. Using the ``setup-counter-processor.sh`` located at ``/scripts/vagrant/setup-counter-processor.sh`` might help you get it installed.)
...
Next you should run Counter Processor to convert the log into a SUSHI report, which is in JSON format. Before running Counter Processor, you need to put a configuration file into place. As a starting point use ``counter-processor-config.yaml`` located at ``/scripts/vagrant/counter-processor-config.yaml`` and edit the file, paying particular attention to the following settings:
