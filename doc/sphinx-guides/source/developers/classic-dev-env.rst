...
See :download:`docker-compose-dev.yml <../../../../docker-compose-dev.yml>` for the version of PostgreSQL to use. Below we use "PGVERSION" to mean the version of PostgreSQL.

On Mac, go to https://www.postgresql.org/download/macosx/ and choose "Interactive installer by EDB" option. When prompted to set a password for the "database superuser (postgres)" just enter "password".

After installation is complete, make a backup of the ``pg_hba.conf`` file like this:

``sudo cp /Library/PostgreSQL/PGVERSION/data/pg_hba.conf /Library/PostgreSQL/PGVERSION/data/pg_hba.conf.orig``

Then edit ``pg_hba.conf`` with an editor such as vi:

``sudo vi /Library/PostgreSQL/PGVERSION/data/pg_hba.conf``
...
In the Finder, click "Applications" then "PostgreSQL PGVERSION" and launch the "Reload Configuration" app. Click "OK" after you see "server signaled".

Next, to confirm the edit worked, launch the "pgAdmin" application from the same folder. Under "Browser", expand "Servers" and double click "PostgreSQL PGVERSION". When you are prompted for a password, leave it blank and click "OK". If you have successfully edited "pg_hba.conf", you can get in without a password.
