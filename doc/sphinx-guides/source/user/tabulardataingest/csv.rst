...
As of the Dataverse 4.7.2 release, we allow ingest of CSV files with commas and line breaks within cells. A string with any number of commas and line breaks enclosed within double quotes is recognized as a single cell. Double quotes can be encoded as 2 double quotes in a row (``""``). 
...
For the dates, the ``yyyy-MM-dd`` format is recognized. 

For the date-time values, the following 2 formats are recognized: 
...
(We are planning to add direct support for tab-delimited files in one of the upcoming releases). 
