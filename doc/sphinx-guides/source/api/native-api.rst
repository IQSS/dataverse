...
- ``url`` - the exact URL to sign, including api version number and all query parameters. Provide it in exactly the form you will use it - percent-encoded or not: the signature is computed over the URL exactly as provided, and the returned signed URL works when used verbatim. (URLs provided in URL-decoded form and later presented percent-encoded also validate, for compatibility with older clients.) The URL must not already contain any of the reserved query parameters ``until``, ``user``, ``method``, ``token`` (these are appended by the signing), ``key`` or ``signed``; such a URL is rejected with a 400 (Bad Request) response.
