...
metadata_fields  string	  Add requested fields in result response for each dataset. Multiple "metadata_fields" parameters can be used to include several fields. The value must be with the form "{metadata_field_set_name}:{field_name}" to include a specific field from a metadata fields set or "{metadata_field_set_name}:*" to include all the fields for a metadata fields set. {field_name} could not be a sub field of a compound field and if {field_name} is a compound field, all sub-fields are included  
===============  =======  ===========
