...
By default, custom terms of data use and access can be specified after selecting "Custom Terms of Use" from the License/Data Use Agreement dropdown on the Terms tab. When ``:AllowCustomTermsOfUse`` is  set to ``false`` the "Custom Terms of Use" item is not made available to the depositor.
