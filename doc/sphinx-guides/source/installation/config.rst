...
Additionally, instance administrators can grant the "Power User" role to specific users on a per-collection or per-dataset basis. This role allows delegated administrators to perform many tasks that were previously restricted to global superusers, such as moving objects, managing PIDs, and configuring storage drivers, without granting them full superuser status.
