...
1. If for some reason it didn't deploy, check the server.log file. It may just be a caching issue so try un-deploying, deleting cache, restarting, and re-deploying on the server (`su - dataverse` then `/usr/local/payara7/bin/asadmin list-applications; /usr/local/payara7/bin/asadmin undeploy dataverse-6.1; /usr/local/payara7/bin/asadmin deploy /tmp/dataverse-6.1.war`)
