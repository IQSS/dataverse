...
# Store the tags of application images we are actually rebuilding
# Takes the from "branch-name=app-image-ref"
REBUILT_APP_IMAGES=()
...
for BRANCH in "$@"; do
  ...
  if [[ -d ${GITHUB_WORKSPACE}/src/backports/$BRANCH ]]; then
    echo "Applying patches now."
    find "${GITHUB_WORKSPACE}/src/backports/$BRANCH" -type f -name '*.patch' -print0 | xargs -0 -n1 patch -p1 -l -s -i
  fi
  ...
  if (( IS_DEV )); then
    BASE_IMAGE_REF=$( mvn initialize help:evaluate -Pct -f . -Dexpression=base.image -q -DforceStdout )
  else
    # Frist, get the rolling tag of the base image
    ROLLING_BASE_REF=$( mvn initialize help:evaluate -Pct -f . -Dexpression=base.image -Dbase.image.tag.suffix="" -q -DforceStdout )
    # Now, we want to build any release branch application images on top of a _fixed_ tag, so let's fetch the newest fixed tag
    CURRENT_REV=$( current_revision "$ROLLING_BASE_REF" )
    BASE_IMAGE_REF="$ROLLING_BASE_REF-r$CURRENT_REV"
  fi
  echo "Determined BASE_IMAGE_REF=$BASE_IMAGE_REF from Maven"

  # 3b. Determine the app image ref (<namespace>/<repo>:<tag>)
  APP_IMAGE_REF=""
  if (( IS_DEV )); then
    # Results in the rolling tag for the dev branch
    APP_IMAGE_REF=$( mvn initialize help:evaluate -Pct -f . -Dexpression=app.image -q -DforceStdout )
  else
    # Results in the rolling tag for the release branch (the fixed tag will be determined from this rolling tag)
    # shellcheck disable=SC2016
    APP_IMAGE_REF=$( mvn initialize help:evaluate -Pct -f . -Dexpression=app.image -Dapp.image.tag='${app.image.version}-${base.image.flavor}' -q -DforceStdout )
  fi
  echo "Determined APP_IMAGE_REF=$APP_IMAGE_REF from Maven"

  # 4. Check for Base image updates
  NEWER_BASE_IMAGE=0
  if check_newer_parent "$BASE_IMAGE_REF" "$APP_IMAGE_REF"; then
    NEWER_BASE_IMAGE=1
  fi

  # 5. Get current immutable revision tag if not on the dev branch
  REV=$( current_revision "$APP_IMAGE_REF" )
  CURRENT_REV_TAG="${APP_IMAGE_REF#*:}-r$REV"
  NEXT_REV_TAG="${APP_IMAGE_REF#*:}-r$(( REV + 1 ))"
  ...
  if (( NEWER_BASE_IMAGE + FORCE_BUILD > 0 )); then
    ...
    if ! (( IS_DEV )); then
      REBUILT_APP_IMAGES+=("$BRANCH=${APP_IMAGE_REF%:*}:$NEXT_REV_TAG")
    else
      REBUILT_APP_IMAGES+=("$BRANCH=$APP_IMAGE_REF")
    fi
  else
  ...
  if ! (( IS_DEV )); then
    ...
    RELEASE_TAGS_LIST+="\"${APP_IMAGE_REF#*:}\", "
    if (( NEWER_IMAGE )); then
...
REBUILT_IMAGES="["
for IMAGE in "${REBUILT_APP_IMAGES[@]}"; do
