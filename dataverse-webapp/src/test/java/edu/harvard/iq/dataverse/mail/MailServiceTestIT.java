...
public class MailServiceTestIT extends WebappArquillianDeployment {
    ...
    public void sendMail_whenOverseerActive() {
        // given
        assertThat(overseerEmail, not(emptyOrNullString()));

        // when
        mailService.sendMail(userMailAddress, null, content);
        Awaitility.await()
                .atMost(10L, TimeUnit.SECONDS)
                .until(() -> !smtpServer.getMails().isEmpty());

        // then
        List<Tuple2<String, String>> mailBox = smtpServer.getMails()
                .stream()
                .map(m -> Tuple.of(m.getTo(), m.getSubject()))
                .collect(Collectors.toList());
        assertThat("Two mails should be present in mailbox", smtpServer.getMails().size(), is(2));
